...
const PricingPage = () => {
 ...
 <div className="flex flex-col md:flex-row justify-center items-center md:items-start gap-6 lg:gap-8">        {[
          ...
          <article
            key={index}
        className={`border-2 border-blue-200 dark:border-rose-100 w-full md:w-2/3 lg:w-${index === 1 ? '2/5' : '1/5'} rounded-lg p-7 flex flex-col justify-between ${card.bgClass} transition-all duration-300 ease-in-out transform ${index === 1 ? `lg:scale-110 shadow-xl ${classes.specialCard}` : 'hover:scale-105'}`}



          >
            {index === 1 && <span className="absolute top-0 right-0 p-2 bg-red-500 text-white text-sm rounded-bl-lg">Best Value!</span>}

            <div>
              <h3 className="font-medium text-2xl text-center mb-5 mt-3">
                {card.title}
              </h3>
            <p className={`${classes.multiLineEllipsis} text-center text-xs leading-5`}>
  {card.description}
</p>
